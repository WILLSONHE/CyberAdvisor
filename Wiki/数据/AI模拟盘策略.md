...
3. **4033/4130 与 4120/4000/3950 的关系**：
   - **4033/4130 / L1–L5**：均在 **[[指数纪律框架]]** 与最新日更中维护；**不由规则引擎硬编码**。
   - Agent 每个 tick 读 Wiki 后，通过 **`buy_permission.allowed`** 与 **`param_changes`**（`EQUITY_TARGET_NORMAL`、`MAX_BUYS_PER_TICK` 等）表达仓位意图。
   - **4120 / 60 分钟底 / 4000→3950 / 缩量**：回补与企稳条件；`analysis` **必须分层写出**，不可只写 4033。
...
| **盘中** | 09:30–11:15、13:00–14:45 每 15 分钟 | `intraday` | 结合 tick 行情；多数 tick **不调参**；有明确环境变化才改 1–3 个参数 |
| **休市 daily** | **11:30**、**15:00** | — | **不跑 sim tick**；计划任务改跑 **`daily.bat`**（持仓同步 + 市场日报） |
...
*最后更新：2026-06-10 · 指数纪律改由 Agent 读 Wiki 调参（非 4033 硬编码）*
