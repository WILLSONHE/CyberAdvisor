...
12. 飞书：`FEISHU_WEBHOOK_URL` 推送 daily 摘要；`feishu_bot.bat` 本机 Bot；Bot 指令：`sug {持有人} [早盘|午盘]`、`sug 全员`、`trk {标的}`、`chk`、`qry {问题}`
...
CLI 测试（与 Bot 同源）：`python scripts/wiki_cli.py trk 寒武纪` / `chk` / `qry 存储`
