...
4. 阅读 `Wiki/博主/博主决策时间线.md` 了解 Wiki 最新操作节奏（输出称 **决策时间线**）
5. 阅读 `Wiki/数据/博主标的池日报.md` 中 **该持有人的做T章节**（输出称 **标的池日报**）
...
**所有面向用户的回答、分析、sug、qry、trk 解读、根目录临时 md 等，正文中不得出现「博主」二字。**

分析须锚定可核查材料，用下列用语替代（按语境择一，可组合）：

| 禁用 | 推荐替代 |
|------|----------|
| 博主 | **Wiki** / **研报** / **日更** / **专栏** / **视频专题** / **Raw 稿** |
| 博主判断 / 博主框架 | **Wiki 框架判断** / **Wiki 投资框架**（链到具体页如 [[指数纪律框架]]） |
| 博主情绪周期 | **情绪周期**（注明来源：[[情绪周期与操作节奏]] 或当日 [[YYYY-MM-DD]]） |
| 博主操作建议 | **Wiki 操作建议** / **日更口径** |
| 博主态度 | **Wiki 追踪口径** / **日更提及** |
| 博主优先级 | **标的池优先级**（`Wiki/数据/博主标的池日报.md`） |
| 博主动态 | **Wiki/Raw 动态** |
| 博主纪律 | **指数纪律** / **Wiki 纪律框架** |
| 博主标的池 | **Wiki 标的池** / **标的池日报** |
| 博主矛盾 | **Wiki 口径矛盾**（触发词仍可识别，输出不用「博主」） |

**写法要求：**
1. 关键结论须 **标明出处**（`[[2026-06-10]]`、`Raw/已分析归档/研报：…`、[[板块轮动记录]] 等）。
2. 目录名 `Wiki/博主/`、脚本变量 `BLOGGER_STOCKS` 等为 **内部路径/代码**，不必改文件夹；**生成给用户看的句子**须按上表替换。
