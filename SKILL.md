...
| 最多持仓标的 | **5** 只 |
| 单标的上限 | 总资金 **25%** |
| 4033/4130 | **软约束**（破 4033 降目标仓位至 ~35%；可偏离但须在日志写理由） |

### 标的池

1. `Wiki/博主/标的追踪/` **活跃**目录（不含 `不活跃/`、`股性-*`）且能在 `fine_screen.BLOGGER_STOCKS` 解析代码  
...
| **盘中** | 09:30–11:30、13:00–15:00 每 15 分钟 | 常规 tick |
| **午休复盘** | **11:45** | 上午复盘 + 下午策略；**可选**调参（每日一次） |
| **收盘复盘** | **15:15** | 全天复盘 + 为次日 09:15 准备 |

计划任务：`scripts/ai_sim_register_tasks.ps1`（共 **21** 个时刻）。**调参非必须**——充分分析后可 `hold_params` 维持不变。
...
6. 与博主视频/专栏内容无直接双链时可链到 `Wiki/市场分析/` 或相关标的页；**勿**混入 `每日复盘/` 除非明确是博主口径
...
7. **归档**：
   - **专栏/动态**：`python scripts/archive_raw.py archive <路径>` → `Raw/已分析归档/`
   - **视频字幕稿**：同上命令，但脚本识别 `type: video_transcript` / 文件名含 `_BV…_` → **仅标记 `ingested`，文件留在 `Raw/已审阅视频文稿/`**（误归档用 `archive_raw.py restore-videos`）
   - 专栏归档时 **自动 track-maintain**
...
| **四、活跃标的追踪** | `Wiki/博主/标的追踪/` 活跃目录（不含 `不活跃标的/`、`股性-*`）全量：**行情明细 + 逐只摘要** |
| **五、市场总结** | 结合当日数据 + `Wiki/市场分析/` + `Wiki/投资方法论/` 的规则对照（4033 纪律、板块轮动、追踪池表现） |

**消费方**：
- `sug … 午盘` / 收盘复盘：**必读第一节指数收盘** + 第五节总结
- `sug … 早盘`：可读前一交易日第五节 + 当日博主 Wiki
...
| 指令 | 含义 | 建议生成时机 | 叙述视角 |
|------|------|------------|---------|
| `sug … 早盘` | **早盘总结** | 上午收盘后（约 11:30 后） | **回顾** 9:30–11:30：盘面、博主早盘/上午动态、持仓涨跌、做 T 是否执行 |
...
**第一章「大盘与博主判断」**：盘次总结须填 **该时段已发生** 的指数收盘/高低、成交额、博主动态（含当日已 ingest 的 Wiki/Raw）。

**第三章持仓**：用 **该时段结束时的现价**（午盘总结 = 收盘价）。

**第四章「能不能开新仓」**：盘次总结侧重 **今日是否已操作/是否应留到次日**；4033 已破则写 **博主纪律已触发 → 应清仓/已清仓/待执行**。
...
3. 阅读最近 2-3 天的博主早盘/复盘/午盘补发 Raw 文件；**盘次总结**须含 **当日** 同时段 Wiki（如 `[[2026-06-05]]`）及已 ingest 动态
