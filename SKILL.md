...
**指令格式（Cursor / 飞书 Bot 一致）**：
- `sug {持有人}` — 例：`sug Wilson`（动词与持有人名均不区分大小写；持有人须与 xlsx 中 **精确匹配**，仅忽略大小写）
- `持仓 {持有人}`、`标的池 {持有人}`、`日报 {持有人}` — 同上
- 格式错误或缺少持有人 → 回复：`请校对格式sug {持有人}，以精确搜索`（其他指令将 `sug` 换为对应动词）
...
## Suggest 操作（`sug {持有人}`）—— 交易策略建议

触发条件：用户说 `sug {持有人}`（如 `sug Wilson`），或带持有人的 `开仓 {持有人}`、`买什么 {持有人}`、`持仓分析 {持有人}`、`交易策略 {持有人}`。

**若仅说 `sug` / `开仓` 等而无持有人** → 直接回复：`请校对格式sug {持有人}，以精确搜索`（勿继续生成报告）。
...
7. **归档**：将完整回复写入 `SugVault/YYYY-MM-DD_{持有人}_sug.md`（同日多次则 `YYYY-MM-DD_HHMM_{持有人}_sug.md`；持有人名用 xlsx 中的 canonical 拼写）
...
12. 飞书：`FEISHU_WEBHOOK_URL` 推送 daily 摘要；`feishu_bot.bat` 本机 Bot；Bot 指令：`sug {持有人}`、`trk {标的}`、`chk`、`qry {问题}`

---

## 飞书 Bot Wiki 指令（轻量版）

本机 `feishu_bot.bat` 运行时可发（**不需 Cursor**）：

| 指令 | 行为 | 与 Cursor AI 版差异 |
|------|------|---------------------|
| `trk {标的}` | 读 `Wiki/博主/标的追踪/{标的}.md` + 标的总览行 + grep 补充 | AI 版可刷新追踪页、态度分析 |
| `chk` | 待 ing 计数、断链抽样、活跃区时效性抽样 | AI 版可修复、降级归档 |
| `qry {问题}` | Wiki 关键词检索 + 片段摘要 | AI 版综合多页作答 |

格式错误：`请校对格式trk {标的}，以精确搜索` / `请校对格式qry {问题}，以精确搜索`

CLI 测试：`python scripts/wiki_cli.py trk 寒武纪` / `chk` / `qry 存储`
