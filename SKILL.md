...
| `ing` | ingest | 消化入 wiki |
| `vipdoc` | vipdoc refresh | **vipdoc 日 K 就绪后**刷新 σ/七轨/1·3·7 批追踪（`scripts/vipdoc_refresh.py`）；**午盘 sug 前必跑** |
...
│   └── vipdoc_refresh.py   ← vipdoc 就绪后批追踪（Cursor: vipdoc）
├── Raw/（原始稿）
│   ├── 未分析归档/          ← ing 输入：手动专栏/动态 md
│   ├── 已分析归档/          ← ing 完成后原稿归档
│   ├── 未分析其他材料/      ← ing 输入：pdf/pptx/docx/txt/md 等
│   ├── 已分析其他材料/      ← 其他材料 ing 完成后原稿归档
│   ├── 未审阅视频文稿/      ← bilibili_fetch / douyin_fetch 写入；rw 后 txtcfm 前
...
- **B 站视频字幕稿** → `bilibili_fetch.py` 自动入 `Raw/未审阅视频文稿/`（**已含补标点**）→ `daily.bat` `rw_video.py` 再校正 → **`txtcfm` 审批** → `ing`
- **抖音 ASR 稿（fetch tiktok / 钱加贝）** → `douyin_fetch.py`（SenseVoice）→ 同上目录与 rw/txtcfm/ing 流程
...
**全员** `sug 全员` 对每位持有人 **分别** 执行 0b→7b 全流程；**`sug 全员 午盘`** 在循环前 **先执行 0a（vipdoc）**。

**daily.bat** 第 6 步在 **15:00** 对 `Wiki/内容源/标的追踪` 执行 `batch --universe track`（复盘到期 + 无历史预测则登记）；**不含**当日 vipdoc 收盘 K 线（.day 通常 **15:45 后**才写入），**午盘 sug 前须再跑 `vipdoc`**。
...
| 步骤 | 脚本 | 说明 |
|------|------|------|
| 1 B 站增量 | `scripts/bilibili_fetch.py` | UP 主新视频 → `Raw/未审阅视频文稿/` |
| 2 抖音增量 | `scripts/douyin_fetch.py` | 钱加贝等 → 同上（ASR）；别名 `tiktok_fetch.py` |
| 2 rw | `scripts/rw_video.py` | ASR 术语校正 + 补标点分段（第 7 步 `--pending-only`） |
| 3 txtcfm | `scripts/txtcfm.py` | 审批 → `Raw/已审阅视频文稿/` |
| 4 ing | 主 agent 或 `video_ingest_batch.py` | 结构化写入 Wiki（双轨：日更 + 视频专题）→ `archive_raw.py archive` |
...
10. B 站：`python scripts/bilibili_fetch.py`（**仅视频字幕**）
11. 抖音：`python scripts/douyin_fetch.py`（**fetch tiktok / SenseVoice**；需 `DOUYIN_COOKIE`）
12. 每日流水线：双击 `daily.bat`（… → bilibili + douyin fetch → **rw_video** → dry-run）
12. 飞书：`FEISHU_WEBHOOK_URL` 推送 daily 摘要；`feishu_bot.bat` 本机 Bot（**内容类指令 → .md 附件**；`sug`/`持仓`/`标的池`/`trk`/`chk`/`qry`/`sim` 等）；`帮助`/`ping`/agent 确认 → 短文本
...
| `sug {持有人}` [早盘\|午盘] | 📎 .md 附件 | ✅ 生成 | **`agent sug …`** = Cloud Agent 异步生成（.md 附件） |
| `sug 全员` | 📎 .md 附件 | ✅ 逐人生成 | **`agent sug 全员 午盘`** |
| `持仓` / `标的池` / `日报 {持有人}` | 📎 .md 附件 | ✅ 读 | 读 `portfolio.md`、标的池日报 |
| `trk {标的}` | 📎 .md 附件 | ✅ 深度 | AI 版可刷新追踪页、态度分析 |
| `chk` | 📎 .md 附件 | ✅ 深度 | AI 版可修复、降级归档 |
| `qry {问题}` | 📎 .md 附件 | ✅ 综合 | **`agent qry {问题}`** 或 **`agent {自由任务}`** = Cloud Agent |
| `sim 买/卖 {标的…}` | 📎 .md 附件 | ✅ | 模拟持仓；sug 前 Cursor 跑 `sim_portfolio.py sync` |
| `帮助` / `ping` | ✅ | — | Bot 专用 |
