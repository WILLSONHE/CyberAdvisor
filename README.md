...
- **Wiki 知识库**：博主 2025.10~2026.05 全部方法论 + 121 天每日复盘 + 标的全追踪（活跃池自动建页，非池归档至 `不活跃标的/`）
...
CLI 自测（与 Bot 同源）：`python scripts/wiki_cli.py trk 寒武纪` / `track-maintain` / `chk` / `qry 存储`
...
│   ├── 每日复盘/
│   ├── 博主/
│   │   └── 标的追踪/          ← 活跃池专页（提及≥3 次自动建页）
│   │       └── 不活跃标的/    ← 已移出 BLOGGER_STOCKS 的历史追踪页
│   └── 数据/                ← 脚本输出
└── scripts/
    ├── feishu_download_portfolio.py  ← 飞书云文档 → 持仓.xlsx
    ├── wiki_cli.py          ← trk / chk / qry / track-maintain（飞书 Bot 共用）
    ├── wiki/                ← Wiki 轻量查询模块（trk/chk/qry/track_manage）
...
| `c244ddd` | **commit 10：ing 归档自动 track-maintain** — `archive_raw.py` archive/migrate 完成后自动同步标的追踪 |
