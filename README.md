...
约 15–20 分钟。完成后对 AI 说 **`sug {持有人}`**（如 `sug Wilson`）或 **`sug 全员`**，得到交易策略报告；未指定盘次写入 `SugVault/YYYY-MM-DD_{持有人}_sug.md`，指定早盘/午盘则写入 `..._sug 早盘.md` / `..._sug 午盘.md`。
...
| **本机 Bot** | `FEISHU_APP_*` + 双击 `feishu_bot.bat` | `sug Wilson [早盘\|午盘]` / `sug 全员` / `持仓 Wilson` / `标的池 Wilson` / `trk 寒武纪` / `chk` / `qry 问题` / `帮助` |
...
| `sim 买/卖 {标的…}` | ✅ | ✅ | 模拟持仓写入 `模拟持仓.xlsx`；sug 前 `python scripts/sim_portfolio.py sync` |
...
CLI 自测（与 Bot 同源）：`python scripts/wiki_cli.py trk 寒武纪` / `chk` / `qry 存储` / `sim-handle "sim 买 利通电子"` / `sim-sync`
...
根目录 **`模拟持仓.xlsx`**，由 `scripts/sim_portfolio.py` 维护（Cursor / 飞书 Bot 均可调用）。

| 指令 | 说明 |
|------|------|
| `sim 买 利通电子，江波龙，…` | 每标的 **100 万起**（不足则 200 万、300 万…整数倍）；成本=买入时市价，股数>100 |
| `sim 卖 利通电子` | 冻结该行，`卖出？(Y/N)=Y`，不再 sync |
| `python scripts/sim_portfolio.py sync` | sug 前刷新现价；**sug 流程第 0 步** |
| `python scripts/sim_portfolio.py rebuild` | 按 100 万规则重算未卖出持仓 |

**列说明**：`成本`×`股数`=单票投资成本；`sync` 只更新 `现价/市值/盈亏`，不改 `成本/股数`。表末**空一行**后 **「合计」** 行汇总：

- **成本**列 = 全部持仓投资成本之和  
- **市值**列 = 全部持仓市值之和  
- **盈亏比** = 组合总体盈亏比 `(市值合计 − 成本合计) / 成本合计`

CLI：`python scripts/sim_portfolio.py handle "sim 买 …"` / `show` / `sync`
...
├── 模拟持仓.xlsx            ← sim 买/卖 模拟盘（scripts/sim_portfolio.py）
...
└── scripts/
    ...
    ├── sim_portfolio.py     ← sim 买/卖/sync → 模拟持仓.xlsx
